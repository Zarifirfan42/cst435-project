...
# Benchmark Function
# ---------------------
def benchmark_parallel(func, workers, numbers):
    n_workers = len(workers)
    chunks = [numbers[i::n_workers] for i in range(n_workers)]
    start = time.perf_counter()

    results = []
    for i, worker in enumerate(workers):
        results.append(func(chunks[i], worker))

    total = sum(results)
    end = time.perf_counter()
    total_time = end - start
    throughput_tx = 1 / total_time  # transactions/sec
    throughput_items = len(numbers) / total_time  # items/sec
...
if __name__ == "__main__":
    ...
    print("=== gRPC Multi-Worker Sequential ===")
    benchmark_parallel(grpc_sum_squares_worker, GRPC_WORKERS, numbers)

    print("=== REST Multi-Worker Sequential ===")
    benchmark_parallel(rest_sum_squares_worker, REST_WORKERS, numbers)

    print("=== XML-RPC Multi-Worker Sequential ===")
    benchmark_parallel(xmlrpc_sum_squares_worker, XMLRPC_WORKERS, numbers)
